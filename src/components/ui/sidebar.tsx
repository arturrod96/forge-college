...
/** When true, the sidebar will not collapse on mouse leave (e.g. while a dropdown is open). */
export const SidebarPreventCollapseContext = React.createContext<{ prevent: boolean }>({ prevent: false })
...
>(
  (
    ...
    const preventCollapse = React.useContext(SidebarPreventCollapseContext).prevent
    ...
    React.useEffect(() => {
      if (preventCollapse && leaveTimeoutRef.current) {
        clearTimeout(leaveTimeoutRef.current)
        leaveTimeoutRef.current = null
      }
    }, [preventCollapse])
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all duration-300 ease-in-out group-data-[side=left]:right-0 group-data-[side=right]:left-0 sm:flex",
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex flex-col gap-2 p-2 transition-[padding] duration-300 ease-in-out", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn(
        "flex flex-col gap-2 p-2 transition-[padding,gap] duration-300 ease-in-out",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "data-[active=true]:bg-[#303b2e] data-[active=true]:text-white",
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        "group-data-[collapsible=icon]:hidden",
        className
